...
function checkForEmptyFormFields(...inputs) {
...
function showErrorMessage(input, msg) {
    ...
    const errorMsg = formFields.querySelector('.error-message');
...
function validateEmailField(email) {
    const isEmail = validator_1.default.isEmail(email.value);
    if (!isEmail)
        showErrorMessage(email, 'Informe um email válido');
}
...
function shouldSubmitForm(form) {
    let send = true;
